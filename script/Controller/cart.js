(function() {
	'use strict';
	angular.module('myApp.cart', ['ngRoute','app.configEnv'])
	...
	angular.module('myApp.cart').controller('cartCtrl', ['$rootScope','$scope', '$location', '$http','$localStorage','CONFIG','$window',
	function($rootScope,$scope,$location,$http,$localStorage,CONFIG,$window) {
		...
		$scope.submit = function(label){
			...
			$scope.switch(function(){
				if (label=='cod') {
				    ...
				    data1.amount=$scope.order.data.total/5;
					...
					$scope.refreshToken(function(resp){
					    $http({
					        ...
					        var options = {
				              ...
				              "description": response.data.request_data.order_name,
		...
		$scope.remFromCart = function(label,cartItem){
			...
			$scope.refreshToken(function(resp){
				...
				$http({
					...
					delete $localStorage['cart'];
					if(label=='places'){
						delete $localStorage['cartIds'][cartItem.cartDetails.place];
						$rootScope.addRemoveMessage = 'Place Has Been removed!';
					}
					if(label=='activities'){
						delete $localStorage['cartIds'][cartItem.cartDetails.activity];
						$rootScope.addRemoveMessage = 'Activity Has Been removed!';
					}
					if(label=='cars'){
						delete $localStorage['cartIds'][cartItem.cartDetails.car];
						$rootScope.addRemoveMessage = 'Car Has Been removed!';
					}
					if(label=='bikes'){
						delete $localStorage['cartIds'][cartItem.cartDetails.bike];
						$rootScope.addRemoveMessage = 'Bike Has Been removed!';
					}
		...
		$scope.changeTaby = function(label){
			// console.log(label);
			if (label=='pay') {
				$scope.changeTab.name='pay';
			}else if (label=='info') {
				$scope.changeTab.name='info';
			}else{
				$scope.changeTab.name='details';
			}
		}
		...
		$scope.changeDate = function(){
			//console.log('here'+new Date($scope.dateItin.dateItinT));
			$scope.dateItin.dateItinT = new Date($scope.dateItin.dateItinT);
		...
		function init(){
			...
			if ($localStorage['date']) {
				...
				$scope.dateItin={};
				$scope.dateItin.dateItinT= new Date();
			}
